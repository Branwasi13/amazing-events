...
const EventHighestStats = document.getElementById("EventHighestStats")
const EventLowestStats = document.getElementById("EventLowestStats")
...
// ---------------- FUNCTIONS ----------------------------
function apiData (){
    fetch("https://amazing-events-d7382-default-rtdb.firebaseio.com/api.json").then(api => api.json()).then(data => {
        ...
        if (EventHighestStats){
            percentageOfAttendance(events)
            printRow(events)
        }
        ...
        if (upcomingStats){
            ...
            asisstance(events)
            tableStats(arrayUpcomingStats, upcomingStats)
        }

        if (pastStats){
            let arrayPastStats = events.filter(event => event.date < date)
            asisstance(events)
            tableStats(arrayPastStats, pastStats)
        }
...
function percentageOfAttendance(array){
    let attendance = []
    ...
    EventLowestStats.innerText = (attendance[attendance.length-1]) + ' %'
    EventHighestStats.innerText = (attendance[0]) + ' %'
}
...
function asisstance(array){
    let categorys = []
    let arrayStats= []
    array.forEach(evento => {
        if(!categorys.includes(evento.category)){
...
function tableStats(array, contenedor){
    let eventStats = asisstance(array)
